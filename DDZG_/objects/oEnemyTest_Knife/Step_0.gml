...
if actualEnemyState!=enemie_state.hurt_{
	collision_mask_body = collision_rectangle(x-32,y+z,x+32,y-50,oSlash,true,true); 
	
	if collision_mask_body!=noone{
		actualEnemyState=enemie_state.hurt_;
		var Hitforce = collision_mask_body.HitForce
		InertiaForce=Hitforce;
		 z-=Hitforce 
		 HurtsndSound=true;
	}
...
y2=noone;
	...
	if instance_exists(oPlayer){
		if hspeed!=0 {
if distance_to_point(oPlayer.x,oPlayer.y)<=50 {hspeed=0;}}
	}
	

	
	sprite_index=sprKnifeAttack;
	_statePrint="ATTACK";
	
	switch (attackDur) {
		case 1: sprite_index=sprKnifeAttack; image_index=0; _stela=true;
		break;
